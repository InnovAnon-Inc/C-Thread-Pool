...
struct thpool_* thpool_init(int num_threads){
	...
	thpool_p->threads = (struct thread**)malloc((size_t) num_threads * sizeof(struct thread *));
...
int thpool_add_work(thpool_* thpool_p, void (*function_p)(void*), void* arg_p){
	...
	/* add job to queue */
	if (jobqueue_push(&thpool_p->jobqueue, newjob) != 0) return -2;
...
int thpool_wait(thpool_* thpool_p){
	if (pthread_mutex_lock(&thpool_p->thcount_lock) != 0) return -1;
	while (thpool_p->jobqueue.len || thpool_p->num_threads_working) {
		if (pthread_cond_wait(&thpool_p->threads_all_idle, &thpool_p->thcount_lock) != 0) return -2;
	}
	if (pthread_mutex_unlock(&thpool_p->thcount_lock) != 0) return -3;
	return 0;
}
...
int thpool_destroy(thpool_* thpool_p){
	...
	while (thpool_p->num_threads_alive){
		if (bsem_post_all(thpool_p->jobqueue.has_jobs) != 0) return -4;
		sleep(1);
	}

	/* Job queue cleanup */
	if (jobqueue_destroy(&thpool_p->jobqueue) != 0) return -5;
	/* Deallocs */
...
int thpool_pause(thpool_* thpool_p) {
	...
	for (n=0; n < thpool_p->num_threads_alive; n++){
		if (pthread_kill(thpool_p->threads[n]->pthread, SIGUSR1) != 0)
			return -1;
	}
	return 0;
}
...
static int thread_init (thpool_* thpool_p, struct thread** thread_p, int id){
	...
	*thread_p = (struct thread*)malloc(sizeof(struct thread));
	...
	if (pthread_create(&(*thread_p)->pthread, NULL, (void *(*) (void *))thread_do, (*thread_p)) != 0)
		return -2;
...
static void* thread_do(struct thread* thread_p){
	...
	if (sigemptyset(&act.sa_mask) != 0) return NULL;
	act.sa_flags = 0;
	...
	if (pthread_mutex_lock(&thpool_p->thcount_lock) != 0) return NULL;
	thpool_p->num_threads_alive += 1;
	if (pthread_mutex_unlock(&thpool_p->thcount_lock) != 0) return NULL;

	while(threads_keepalive){

		if (bsem_wait(thpool_p->jobqueue.has_jobs) != 0) return NULL;
		...
		if (threads_keepalive){
			...
			if (!thpool_p->num_threads_working) {
				if (pthread_cond_signal(&thpool_p->threads_all_idle) != 0) return NULL;
			}
			if (pthread_mutex_unlock(&thpool_p->thcount_lock) != 0) return NULL;
...
static int jobqueue_init(jobqueue* jobqueue_p){
	...
	if (pthread_mutex_init(&(jobqueue_p->rwmutex), NULL) != 0) return -2;
	if (bsem_init(jobqueue_p->has_jobs, 0) != 0) return -3;
...
static int jobqueue_clear(jobqueue* jobqueue_p){
	...
	jobqueue_p->front = NULL;
	jobqueue_p->rear  = NULL;
	if (bsem_reset(jobqueue_p->has_jobs) != 0) return -2;
	jobqueue_p->len = 0;
	return 0;
}
...
static int jobqueue_push(jobqueue* jobqueue_p, struct job* newjob){
	...
	if (pthread_mutex_lock(&jobqueue_p->rwmutex)) return -1;
	newjob->prev = NULL;
	...
	if (bsem_post(jobqueue_p->has_jobs) != 0) return -2;
	if (pthread_mutex_unlock(&jobqueue_p->rwmutex) != 0) return -3;
	return 0;
}
...
static struct job* jobqueue_pull(jobqueue* jobqueue_p){
	...
	if (pthread_mutex_lock(&jobqueue_p->rwmutex) != 0) return NULL;
	...
	switch(jobqueue_p->len){
		...
		default: /* if >1 jobs in queue */
					...
					if (bsem_post(jobqueue_p->has_jobs) != 0) return NULL;

	}

	if (pthread_mutex_unlock(&jobqueue_p->rwmutex) != 0) return NULL;
	return job_p;
}
...
static int jobqueue_destroy(jobqueue* jobqueue_p){
	if (jobqueue_clear(jobqueue_p) != 0) return -1;
...
static int bsem_post(bsem *bsem_p) {
	if (pthread_mutex_lock(&bsem_p->mutex) != 0) return -1;
	bsem_p->v = 1;
	if (pthread_cond_signal(&bsem_p->cond) != 0) return -2;
	if (pthread_mutex_unlock(&bsem_p->mutex) != 0) return -3;
	return 0;
}
...
static int bsem_post_all(bsem *bsem_p) {
	if (pthread_mutex_lock(&bsem_p->mutex) != 0) return -1;
	bsem_p->v = 1;
	if (pthread_cond_broadcast(&bsem_p->cond) != 0) return -2;
	if (pthread_mutex_unlock(&bsem_p->mutex) != 0) return -3;
	return 0;
}
...
static int bsem_wait(bsem* bsem_p) {
	if (pthread_mutex_lock(&bsem_p->mutex) != 0) return -1;
	while (bsem_p->v != 1) {
		if (pthread_cond_wait(&bsem_p->cond, &bsem_p->mutex) != 0) return -2;
	}
	bsem_p->v = 0;
	if (pthread_mutex_unlock(&bsem_p->mutex) != 0) return -3;
	return 0;
